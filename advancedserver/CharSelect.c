...
bool charselect_check_state(Server* server)
{
	...
	if (should) {
		if ((g_config.character_selection.charselect_mod_unlocked && !g_config.gameplay.hide_player_characters) || !g_config.character_selection.enable)
			for (size_t i = 0; i < server->peers.capacity; i++)
			{
				PeerData* peer = (PeerData*)server->peers.ptr[i];
				if (!peer)
				...
				PacketWrite(&pack, packet_write8, peer->id == server->lobby.exe ? peer->exe_char : peer->surv_char);
...
{
	if(!g_config.character_selection.enable)
		return charselect_check_state(v->server);
	...
	case CLIENT_REQUEST_EXECHARACTER:
	{
		...
		if (!g_config.gameplay.hide_player_characters && !g_config.character_selection.charselect_mod_unlocked) {
			PacketCreate(&pack, SERVER_LOBBY_CHARACTER_CHANGE);
