...
bool game_init(int exe, int8_t map, Server* server)
{
    ...
    RAssert(server_ingame(server) > 1 - g_config.gameplay.banana.singleplayer);
    ...
    if (!dylist_create(&server->game.left, g_config.pairing.maximum_players_per_lobby))
		return false;
	...
	for (size_t i = 0; i < server->peers.capacity; i++)
	{
        ...
        if(g_config.gameplay.anticheat.ability_anticheat)
...
bool game_checkstart(Server* server)
{
	...
	if (cnt >= server_ingame(server))
	{
		...
		if (server->game.time_sec > g_config.gameplay.gametimers_ceiling)
			server->game.time_sec = g_config.gameplay.gametimers_ceiling;
...
bool game_demonize(Server* server, PeerData* data)
{
    ...
    if (players * (g_config.gameplay.demonization_percentage / 100.0) > demonized)
	{
...
bool game_state_handletcp(PeerData* v, Packet* packet)
{
	...
	switch (type)
	{
        ...
        {
            if(g_config.gameplay.enable_achievements){
                AssertOrDisconnect(v->server, v->in_game);
                server_broadcast_ex(v->server, packet, true,  v->id);
            }
            break;
        ...
        {
            if(g_config.gameplay.enable_sounds){
                AssertOrDisconnect(v->server, v->in_game);
                server_broadcast_ex(v->server, packet, true,  v->id);
            }
            break;
		...
		{
			AssertOrDisconnect(v->server, v->in_game);
            AssertOrDisconnect(v->server, !g_config.gameplay.anticheat.palette_anticheat || palette_player_validate(v, packet));
			server_broadcast_ex(v->server, packet, true,  v->id);
			break;
		}
		...
		case CLIENT_PLAYER_HEAL_PART:
		{
			...
			if (g_config.gameplay.anticheat.data_based_anticheat && rings < 10)
			{
				server_disconnect(v->server, v->peer, DR_OTHER, "pusy");
				return true;
			}

			if (g_config.gameplay.anticheat.data_based_anticheat && rings >= 140 && v->server->game.map != 20)
			{
		...
		case CLIENT_TPROJECTILE:
		{
            ...
            if(v->plr.cooldown > 0 && g_config.gameplay.anticheat.ability_anticheat)
			{
				char msg[256];
			...
			if (v->plr.flags & PLAYER_DEMONIZED)
			{
                ...
                if (dmg >= 60 && g_config.gameplay.anticheat.ability_anticheat)
				{
			...
			else
			{
                ...
                if (dmg >= 6 && g_config.gameplay.anticheat.ability_anticheat)
				{
                    double last = time_end(&v->plr.tails_last_proj);
					check_balls = last > 1500 && last < 12000;
					AssertOrDisconnect(v->server, check_balls);
		...
		case CLIENT_PLAYER_DEATH_STATE:
		{
			...
			if (dead)
			{
                ...
                if (v->plr.flags & PLAYER_REVIVED || (v->server->game.time_sec < g_config.gameplay.sudden_death_timer) == !g_config.gameplay.banana.disable_timer)
				{
				...
				else
				{
					...
					if (g_config.gameplay.match_respawn_and_game_timers) {
						uint16_t time_to_sudden_death = g_config.gameplay.banana.disable_timer
							? g_config.gameplay.sudden_death_timer - v->server->game.time_sec
							: v->server->game.time_sec - g_config.gameplay.sudden_death_timer;

						if (time_to_sudden_death < g_config.gameplay.respawn_time) {
							v->plr.death_timer_sec = time_to_sudden_death;
						} else {
							v->plr.death_timer_sec = g_config.gameplay.respawn_time;
						}
					} else {
						v->plr.death_timer_sec = g_config.gameplay.respawn_time;
					}
                    ...
                    PacketWrite(&pack, packet_write8, exe && vector2_dist(&v->plr.pos, &exe->plr.pos) <= 240 && g_config.gameplay.exe_camp_penalty);
					PacketWrite(&pack, packet_write16, v->id);
		...
		case CLIENT_PLAYER_DATA:
		{
			...
			if(v->server->game.exe != v->id)
			{
				...
				if(!(v->plr.flags & PLAYER_DEAD) && !(v->plr.flags & PLAYER_DEMONIZED))
				{
					if (v->server->game.exe != v->id)
					{
						...
						if (revival < 2)
						{
							if (rings < 0 || (v->server->game.map != 20 && rings >= 120) && g_config.gameplay.anticheat.data_based_anticheat)
							{
								server_disconnect(v->server, v->peer, DR_OTHER, "gomunkulus");
								return true;
							}

							if (hp > 100 && g_config.gameplay.anticheat.data_based_anticheat)
							{
            ...
            if(g_config.gameplay.anticheat.distance_anticheat)
			{
			...
			if (v->plr.mod_tool)
			{
				...
				else
				{
					...
					else if (dist >= 300 && g_config.gameplay.anticheat.data_based_anticheat)
					{
						// purpl men
...
bool game_player_tick(Server* server)
{
	...
	for(size_t i = 0; i < server->peers.capacity; i++)
	{
		...
		if (server->delta < 2.5)
		{
			...
			if (data->plr.ping_timer >= 20 * TICKSPERSEC)
			{
				...
				if (avg_ping >= g_config.pairing.ping_limit)
				{
					char msg[130];
	...
	// Start demonization
    if (!server->game.sudden_death && (server->game.time_sec <= g_config.gameplay.sudden_death_timer) == !g_config.gameplay.banana.disable_timer)
	{
	...
	for (size_t i = 0; i < server->peers.capacity; i++)
	{
		...
		if (data->plr.flags & PLAYER_DEAD && data->plr.death_timer_sec > 0)
		{
			...
			for (size_t j = 0; j < server->peers.capacity; j++)
			{
                ...
                if(!g_config.gameplay.exe_camp_penalty)
                    continue;
            ...
            if ((server->game.time_sec < g_config.gameplay.sudden_death_timer) != g_config.gameplay.banana.disable_timer)
			{
			...
			if (data->plr.death_timer >= TICKSPERSEC)
			{
				if (g_config.gameplay.match_respawn_and_game_timers) {
					uint16_t time_to_sudden_death = g_config.gameplay.banana.disable_timer
						? g_config.gameplay.sudden_death_timer - data->server->game.time_sec
						: data->server->game.time_sec - g_config.gameplay.sudden_death_timer;

					if (time_to_sudden_death < g_config.gameplay.respawn_time) {
						data->plr.death_timer_sec = time_to_sudden_death;
					}
				}
...
bool game_state_tick(Server* server)
{
	...
	if (server->game.time <= TICKSPERSEC)
	{
        ...
        g_config.gameplay.banana.disable_timer ? server->game.time_sec++ : server->game.time_sec--;
		...
		if (server->game.time_sec <= 0)
            RAssert(game_end(server, g_config.gameplay.gmcycle.overhell ? ED_SURVWIN : ED_TIMEOVER, true));
	}
