...
bool ban_check(const char* nickname, const char* udid, const char* ip, bool* result)
{
    *result = false;
    MutexLock(g_banMut);
    {
        if (g_config.lobby_misc.moderation.ban_ip) {
            if (cJSON_HasObjectItem(g_banned_ips, ip))
                *result = true;
        }

        if (g_config.lobby_misc.moderation.ban_udid) {
            if (cJSON_HasObjectItem(g_banned_udids, udid))
                *result = true;
        }

        if (g_config.lobby_misc.moderation.ban_nickname) {
            if (cJSON_HasObjectItem(g_banned_nicknames, nickname))
                *result = true;
        }
    }
...
bool op_add(const char* nickname, const char* ip)
{
    ...
    MutexLock(g_opMut);
    {
        if (!cJSON_HasObjectItem(g_ops, ip))
        {
            ...
            cJSON_AddItemToArray(op_array, cJSON_CreateNumber(g_config.lobby_misc.moderation.op_default_level));
