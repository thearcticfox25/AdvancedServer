...
impl DtStalactits {
    pub fn new(stalactite_id: u8, x: u16, y: u16) -> Self {
        Self {
            id: 0,
            stalactite_id,
            state: false,
...
impl Entity for DtStalactits {
    ...
    fn dt_sid(&self) -> i16 { self.stalactite_id as i16 }
    ...
    fn on_init(&mut self, ctx: &mut EntityCtx) -> bool {
        ...
        let _ = pkt.write_u8(self.stalactite_id);
...
impl DtStalactits {
    pub fn activate(&mut self, ctx: &mut EntityCtx) {
        ...
        let cfg = crate::config::cfg();
        let stalactite = &cfg.states.gameplay.entities_misc.map_specific.dark_tower.stalactites;
        let offset = if stalactite.timer_offset == 0 {
            0u64
        } else {
            ctx.rand.gen_range(0..stalactite.timer_offset as u64)
        };

        self.show = false;
        self.state = false;
        self.timer = (stalactite.timer as f64 + offset as f64) * TICKS_PER_SEC;
        self.pos.1 = self.spawn_y as f32;
        self.vel = 0.0;
